pub mod trans {
    ...
    pub async fn fix_punctuation_with_openai(text: &str) -> Result<String, Box<dyn Error>> {
        ...
        let system_message = ChatCompletionRequestMessage {
            role: Role::System,
            content: Some(
                "You are a punctuation restoration assistant. Add punctuation (periods, commas, question marks, exclamation points) and fix capitalization where needed to make the text readable. Return ONLY the corrected text without any extra commentary. Preserve the original wording and do not add or remove content."
                    .to_string(),
            ),
            name: None,
            function_call: None,
